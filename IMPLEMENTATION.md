...
| Mode / route | Imprint | Accent | Pull-quote form |
|---|---|---|---|
...
Each mode holds an ordered list of articles. `/[mode]` is the **imprint index** (nameplate +
article list, emitting `CollectionPage`+`ItemList`); `/[mode]/[slug]` is the **article page**.
`/` is the **The Vigilant Press cover** surfacing the three imprints' featured pieces. All routes
are statically generated via `generateStaticParams` (indexes from the modes, articles from every
`(mode, slug)` pair).
...
app/
  ...
  [mode]/page.tsx       # imprint index — validates mode, lists the mode's articles + CollectionPage/ItemList
  [mode]/[slug]/page.tsx # the article page — validates (mode, slug), composes all components + Article/ClaimReview
components/
  masthead.tsx        # ModeBadge, ModeNav, EditionStrip, Nameplate, CompoundRule
  mode-index.tsx      # ModeIndex — the per-imprint article teaser list
  front.tsx           # FrontMasthead + ImprintSection (the cover)
...
lib/
  modes.ts            # NV_MODES config + NV_ARTICLES (Record<Mode, Article[]>) + accessors + types (single source of truth)
  jsonld.ts           # SPEC-005 builders: buildArticleLd / buildModeIndexLd / buildSiteLd
styles/
...
- **Structured data is emitted per NV-PUB-SPEC-005 (v1.0).** Each page server-renders one
  ...
  `alternateName`); source credibility is not folded in (§4/§7.3, on-page only). Each `/[mode]` index
  emits a `CollectionPage`+`ItemList` of its articles (§5/§7.1 — a real index now exists); `/` stays a
  single minimal `WebSite` node. Author `Person.url` is omitted until `/author/{slug}` pages exist (§7.2).
